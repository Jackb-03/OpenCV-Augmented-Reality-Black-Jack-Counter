...
for Name in [
    ...
    if i <= 13:  # Isolate rank
        ...
        dummy, rank_cnts, hier = cv2.findContours(
            rank, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    if i > 13:  # Isolate suit
        ...
        dummy, suit_cnts, hier = cv2.findContours(
            suit, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
